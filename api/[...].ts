...
function requireAuth(req: AuthedRequest, res: VercelResponse, next: express.NextFunction) {
...
function requireAdmin(req: AuthedRequest, res: VercelResponse, next: express.NextFunction) {
  requireAuth(req, res, () => {
...
function tryAuth(req: AuthedRequest, _res: VercelResponse, next: express.NextFunction) {
  const header = req.headers.authorization;
  if (header && header.startsWith('Bearer ')) {
    try {
...
function validateWeeklySchedule(input: any) {
...
// AUTH
app.post('/api/auth/login', async (req: AuthedRequest, res: VercelResponse) => {
  ...
  const user = {
    ...
    roleType: (spec.roleType as any) || 'professional',
  };
  const token = jwt.sign(user, JWT_SECRET, { expiresIn: '12h' });
  res.json({ token, user });
});

app.get('/api/auth/me', requireAuth, (req: AuthedRequest, res: VercelResponse) => {
  res.json({ user: req.user });
});

app.get('/api/db-status', (req, res: VercelResponse) => {
  res.json({
...
// SERVICES
app.get('/api/services', async (req, res: VercelResponse) => {
  const services = await getServices();
  res.json(services);
});

app.post('/api/services', requireAdmin, async (req, res: VercelResponse) => {
...
app.delete('/api/services/:id', requireAdmin, async (req, res: VercelResponse) => {
...
// SPECIALISTS
app.get('/api/specialists', async (req, res: VercelResponse) => {
...
app.post('/api/specialists', requireAdmin, async (req, res: VercelResponse) => {
...
app.delete('/api/specialists/:id', requireAdmin, async (req, res: VercelResponse) => {
  const id = req.params.id;
  await deleteSpecialist(id);
  res.json({ success: true, id });
});

app.put('/api/specialists/me/schedule', requireAuth, async (req: AuthedRequest, res: VercelResponse) => {
...
// BOOKINGS
app.get('/api/bookings', tryAuth, async (req: AuthedRequest, res: VercelResponse) => {
...
app.post('/api/bookings', async (req, res: VercelResponse) => {
...
app.patch('/api/bookings/:id/status', requireAuth, async (req: AuthedRequest, res: VercelResponse) => {
  const id = req.params.id;
...
// TRANSACTIONS
app.get('/api/transactions', tryAuth, async (req: AuthedRequest, res: VercelResponse) => {
...
app.post('/api/transactions', requireAdmin, async (req, res: VercelResponse) => {
...
app.patch('/api/transactions/:id', requireAdmin, async (req, res: VercelResponse) => {
  const id = req.params.id;
...
app.delete('/api/transactions/:id', requireAdmin, async (req, res: VercelResponse) => {
  const id = req.params.id;
...
// Clients
app.get('/api/clients', requireAuth, (req: AuthedRequest, res: VercelResponse) => {
  if (req.user?.roleType !== 'admin') {
    return res.status(403).json({ error: 'Acesso restrito ao administrador' });
  }
  res.json(clients);
...
app.post('/api/clients', requireAuth, (req: AuthedRequest, res: VercelResponse) => {
  if (req.user?.roleType !== 'admin') {
    return res.status(403).json({ error: 'Acesso restrito ao administrador' });
  }
...
app.put('/api/clients/:id', requireAuth, (req: AuthedRequest, res: VercelResponse) => {
  const { id } = req.params;
...
app.delete('/api/clients/:id', requireAuth, (req: AuthedRequest, res: VercelResponse) => {
  const { id } = req.params;
  const idx = clients.findIndex(c => c.id === id && c.professionalId === req.user!.id);
  if (idx === -1) return res.status(404).json({ error: 'Cliente não encontrado' });
